...
The API creates the database and applies migrations on startup via `DatabaseBootstrapper`.
...
# EF migrations (if adding schema changes)
...
| Migration warnings on startup | Bootstrapper falls back to runtime schema initializers; check API logs |
