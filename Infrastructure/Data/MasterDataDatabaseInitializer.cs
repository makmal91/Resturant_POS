...
public static class MasterDataDatabaseInitializer
{
    public static async Task EnsureSchemaAsync(POSDbContext context, ILogger logger)
    {
        var batches = new[]
        {
            ...
            // Seed default expense categories per active branch
            """
            ...
            // Create categories from legacy free-text CategoryName values
            """
            ...
            // Fallback unmapped expenses to "Other"
            """
            UPDATE e SET e.[ExpenseCategoryId] = ec.[Id]
            FROM [dbo].[Expenses] e
            INNER JOIN [dbo].[ExpenseCategories] ec
            ...
            BEGIN
                ...
                BEGIN
                    UPDATE [dbo].[Expenses] SET [ExpenseCategoryId] = 1 WHERE [ExpenseCategoryId] IS NULL;
                ...
                IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[Expenses]') AND name = N'CategoryName')
                    ALTER TABLE [dbo].[Expenses] DROP COLUMN [CategoryName];
            END
            """,
            ...
            // Migrate existing free-text City to CityId where possible
            """
        ...
        foreach (var batch in batches)
        {
            ...
            {
                logger.LogWarning(ex, "MasterData schema batch skipped or partially applied.");
            }
        }
    }
}
