...
public interface IInvoicePaymentService
{
    ...
    Task<InvoicePaymentDto> ReversePaymentAsync(ReversePaymentDto dto);
    Task<InvoicePaymentDto> UpdatePaymentAsync(int paymentId, UpdatePaymentDto dto);
    ...
    Task<InvoicePaymentDto?> GetPaymentByIdAsync(int paymentId, int businessId, int branchId);
    ...
    Task<PagedResultDto<InvoicePaymentDto>> ListPaymentsAsync(PaymentListFilterDto filter);
}
